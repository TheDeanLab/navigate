...
def Brenner_Measure(input_array, verbose=False):
    ...
    accumulator /= length
    if (verbose):
        print("**** Brenner Measure ****")
    return(accumulator)

def VollathF4(input_array, verbose=False):
    normalized_array = normalize_norm_L1(input_array)
    length = len(normalized_array)
    (width, height) = input_array.shape
    width = int(width /3)
    ...
    while yi < height * width:
        ...
        while x < width - 2:
            i = int(yi + x)
            value = normalized_array[i] * (normalized_array[i + 1] - normalized_array[i + 2])
            accumulator += value 
            x+= 1
        yi += width
    accumulator /= length
