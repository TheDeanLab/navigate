...
from view.notebooks.tabs.channels.channel_settings import channels_label_frame, channel_frame
from view.notebooks.tabs.channels.stack_settings import stack_acq_frame
from view.notebooks.tabs.channels.stack_cycling_settings import stack_cycling_frame
from view.notebooks.tabs.channels.stack_timepoint_settings import stack_timepoint_frame
from view.notebooks.tabs.channels.multipoint_settings import multipoint_frame, multipoint_list
...
class channels_tab(ttk.Frame):
    def __init__(self, setntbk, *args, **kwargs):
        ...
        self.stack_acq_frame.grid(row=2, column=0, columnspan=5, sticky=(NSEW), pady=10)

        #Stack Cycling Settings
        self.stack_cycling_frame = stack_cycling_frame(self)
        self.stack_cycling_frame.grid(row=3, column=0, columnspan=5, sticky=(NSEW), pady=10)

        #Time Settings
        self.stack_timepoint_frame = stack_timepoint_frame(self)
        self.stack_timepoint_frame.grid(row=4, column=0, columnspan=5, sticky=(NSEW), pady=10)

        #Multipoint Enable
        self.multipoint_frame = multipoint_frame(self)
        self.multipoint_frame.grid(row=5, column=0, columnspan=5, sticky=(NSEW), pady=10)

        #Multipoint List
        self.multipoint_list = multipoint_list(self)
        self.multipoint_list.grid(row=6, column=0, columnspan=5, sticky=(NSEW), pady=10)
