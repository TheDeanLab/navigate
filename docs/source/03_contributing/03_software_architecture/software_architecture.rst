...
- **Model**: The model operates in its own subprocess and is responsible for communicating with hardware and performing image handling and processing tasks. Communication with the controller is managed through event queues, ensuring efficient data handling. We also have a specialized model, the **ASIModel**, which is tailored for interfacing with ASI hardware and is described in detail :ref:`here <asi-model-section>`.
