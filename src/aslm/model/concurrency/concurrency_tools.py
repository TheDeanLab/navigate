...
class CustodyThread(ResultThread):
    ...
    def __init__(
        ...
        """Initialize the CustodyThread

        Parameters
        ----------
        first_resource : NoneType
            None
        group : NoneType
            None
        target : NoneType
...
class ObjectInSubprocess:
    """An object that lives in a child process, but acts like it doesn't.
    ...
    def __init__(
        ...
        **initkwargs,
        ...
        """Initialize the ObjectInSubprocess
    ...
    def __setattr__(self, name, value):
        """Set attributes of the child-process object in the parent process.
...
def _get_response(object_in_subprocess, release=False):
    ...
    Parameters
    ----------
    object_in_subprocess : ObjectInSubprocess
        The object in the subprocess
    release : bool
        Whether to release the resource
...
def _child_loop(
    ...
    ------
    Exception
        Exception raised by the child process
...
class _ObjectInSubprocessPipeLock:
    ...
    def __init__(self):
        """Initialize the _ObjectInSubprocessPipeLock"""
        self.lock = threading.Lock()

    def __enter__(self):
        """Enter the _ObjectInSubprocessPipeLock
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        """Exit the _ObjectInSubprocessPipeLock
