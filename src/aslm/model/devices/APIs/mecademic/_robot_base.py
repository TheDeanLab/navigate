...
def disconnect_on_exception(func):
    """Decorator to call disconnect if an exception is raised. Needs to be declared
    outside of class.
...
class _RobotEvents:
    ...
    def abort_all_on_error(self, message=""):
        ...
        self.abort_all(
            skipped_events=[
                "on_connected",
                # Don't abort a wait for "on_connected" (should be done by now anyways)
                "on_deactivated",
                # Don't abort waiting for "on_deactivated".
                # WaitDeactivated is can be used in error
                "on_status_updated",
                # Don't abort a wait for "on_status_updated", that's what we're doing!
                "on_error_reset",
                # Don't abort a wait for "on_error_reset" because we got an error
                "on_end_of_cycle",
                # Don't abort a wait for "on_end_of_cycle", cycles should continue
                # during error
                "on_activate_recovery_mode",
                # Don't abort wait for "on_activate_recovery_mode", available in error
                "on_deactivate_recovery_mode",
                # Don't abort wait for "on_deactivate_recovery_mode", available in error
                "on_estop_reset",
                # The EStop state may still change while in error
                "on_pstop2_resettable",
                # The "PStop2 resettable" state may still change while in error
...
class _CallbackQueue:
    """Queue class for storing triggered callbacks. Only registered callbacks are added
     to the queue.
...
class _Robot:
    ...
    Attributes (private, please use public methods instead, i.e. methods not starting
    with underscore)
    ----------
    _address : string
    ...
    _robot_rt_data : RobotRtData object
        ...
        All attributes of this object are the latest captured on monitor port, so they
        don't necessarily share the same
        timestamp
    _robot_rt_data_stable : RobotRtData object
        Stores most current robot real-time data, but all attributes of object share the
         same timestamp
    ...
    _gripper_state_before_last_move : GripperState object
        ...
        (then used to accelerate WaitGripperMoveCompletion in case target_pos_reached
        is already True when its called
         because otherwise it's not possible to know if the move has completed, or not
         yet started)
    ...
    _file_logger : RobotDataLogger object
        Collects RobotInformation, all RobotRtData and SentCommands during determined
        period
    ...
    _rx_sync : integer
        Most recent response to "SyncCmdQueue" (mx_st.MX_ST_SYNC_CMD_QUEUE) received
        from the robot"""

    def __init__(self):
        """Constructor for an instance of the Robot class."""
        # Initialize member variables that are NOT reset by "with" block (i.e. by
    ...
    def __enter__(self):
        ...
        if self.IsConnected():
            raise InvalidStateError(
                "Robot cannot be connected when entering 'with' block"
            )
        return self
    ...
    @staticmethod
    def _handle_socket_rx(
        robot_socket: socket.socket, rx_queue: queue.Queue, logger: logging.Logger
    ):
    ...
    @staticmethod
    def _handle_socket_tx(
        robot_socket: socket.socket, tx_queue: queue.Queue, logger: logging.Logger
    ):
    ...
    @staticmethod
    def _connect_socket(
        logger: logging.Logger, address: str, port: int, socket_timeout=0.1
    ) -> socket.socket:
        ...
        for _ in range(connect_loops):
            ...
            try:
                ...
                logger.debug(
                    f"Timeout connecting to {address}:{port}, retrying in 100ms."
                )
                continue
            except ConnectionRefusedError:
                logger.debug(
                    f"Connection refused to {address}:{port}, retrying in 100ms."
                )
                time.sleep(0.1)
                continue
            except Exception as exception:
                raise TimeoutError(
                    f"Unable to connect to {address}:{port}, exception: {exception}"
                )
    ...
    def _handle_callbacks(
        ...
        while True:
            ...
            callback_name, data = callback_queue.get(
                block=block_on_empty, timeout=timeout
            )
    ...
    def RegisterCallbacks(
        self, callbacks: RobotCallbacks, run_callbacks_in_separate_thread: bool
    ):
    ...
    def _start_callback_thread(self):
        if self._run_callbacks_in_separate_thread and self._callback_thread is None:
            ...
            self._callback_thread.setDaemon(
                True
            )  # Make sure thread does not prevent application from quitting
    ...
    def Connect(
        ...
        try:
            with self._main_lock:
                ...
                if self.IsConnected():
                    try:
                        ...
                        self.logger.info(
                            "Connection to robot was lost, attempting re-connection."
                        )
                ...
                if len(addr_port) > 1:
                    ...
                    address = addr_port[0]
                    self.logger.info(
                        f"Connecting to robot {address}:{self._custom_port}"
                    )
                else:
            ...
            if not self._monitor_mode and self._robot_info.version.major >= 8:
                ...
                if can_query_robot_info:
                    ...
                    if self._robot_info.rt_message_capable:
                        ...
                        self._monitoring_interval = float(
                            monitoring_interval_response.data
                        )
                    ...
                    if self._robot_info.rt_on_ctrl_port_capable:
                        connect_to_monitoring_port = (
                            False  # We won't need to connect to monitoring port
                        )
                        self._send_custom_command(
            ...
            if self._robot_info.version.major < 8:
                self.logger.warning(
                    "Python API not supported for firmware under version 8"
                )
    ...
    def GetStatusRobot(
        self, synchronous_update: bool = False, timeout: float = None
    ) -> RobotStatus:
        ...
        if synchronous_update:
            ...
            self._send_sync_command(
                "GetStatusRobot", self._robot_events.on_status_updated, timeout
            )
    ...
    def GetStatusGripper(
        ...
        if synchronous_update:
            self._send_sync_command(
                "GetStatusGripper",
                self._robot_events.on_status_gripper_updated,
                timeout,
            )
    ...
    def WaitGripperMoveCompletion(self, timeout: Optional[float]):
        ...
        pos_reached_seen_false = (
            not self._gripper_state_before_last_move.target_pos_reached
        )
        ...
        while current_time < timeout_time:
            ...
            self.logger.debug(
                f"WaitGripperMoveCompletion: Waiting for {wait_duration}s"
            )
            try:
                ...
                with self._main_lock:
                    ...
                    self.logger.debug(
                        f"WaitGripperMoveCompletion: New state is {str(gripper_state)}"
                    )
                ...
                if waiting_move_start:
                    if gripper_state.target_pos_reached:
                        if pos_reached_seen_false:
                            self.logger.debug(
                                f"WaitGripperMoveCompletion: target_pos_reached"
                            )
                            return
                        if (
                            gripper_state.opened
                            and not self._gripper_state_before_last_move.opened
                        ):
                            self.logger.debug(
                                f"WaitGripperMoveCompletion: now opened (was not)"
                            )
                            return
                        if (
                            gripper_state.closed
                            and not self._gripper_state_before_last_move.closed
                        ):
                            self.logger.debug(
                                f"WaitGripperMoveCompletion: now closed (was not)"
                            )
                            return
                    ...
                    if gripper_state.holding_part is False:
                        self.logger.debug(
                            f"WaitGripperMoveCompletion: holding_part_seen_false"
                        )
                        holding_part_seen_false = True

                    if gripper_state.target_pos_reached is False:
                        self.logger.debug(
                            f"WaitGripperMoveCompletion: pos_reached_seen_false"
                        )
                ...
                if waiting_move_start:
                    self.logger.debug(
                        f"WaitGripperMoveCompletion: start_move_timeout reached"
                    )
                    gripper_state = self._gripper_state
                    if gripper_state.target_pos_reached or gripper_state.holding_part:
        ...
        if not gripper_state.target_pos_reached and not gripper_state.holding_part:
            ...
            raise TimeoutException(
                "Timeout while waiting for gripper to complete movement."
            )
    ...
    def UpdateRobot(
        ...
        if address is None and not self.IsConnected():
            raise ArgumentError(
                None, f"address parameter can't be None and not connected to a robot."
            )
        if address is None:
        ...
        if not self.IsConnected():
            ...
            self.logger.info(
                f"Connected to robot in monitoring mode only, attempting connection in command mode"
                "to deactivate robot"
            )
            self.Connect(address=address)
        ...
        if self.GetStatusRobot(synchronous_update=True).activation_state:
            self.logger.info(
                f"Robot is activated, will attempt to deactivate before updating firmware"
            )
        ...
        self.Connect(
            address, timeout=60, enable_synchronous_mode=current_synchronous_mode
        )
        ...
        if str(current_version) in expected_version:
            ...
            error_msg = (
                f"Fail to install robot properly. current version {current_version}, "
    ...
    def _check_monitor_threads(self):
        ...
        if not (
            self._monitor_handler_thread and self._monitor_handler_thread.is_alive()
        ):
            raise InvalidStateError(
                "Monitor response handler thread has unexpectedly terminated."
            )
    ...
    def _check_command_threads(self):
        ...
        if not (
            ...
            and self._command_response_handler_thread.is_alive()
        ):
            raise InvalidStateError(
                "No command response handler thread, are you in monitor mode?"
            )
        ...
        try:
            if self._monitor_mode:
                if not refresh_monitoring_mode:
                    raise InvalidStateError(
                        "Cannot send command while in monitoring mode."
                    )
            else:
    ...
    def _split_command_args(
        self, command: str, args: Union[str, list, tuple] = None
    ...
    def _send_sync_command(
        self, command: str, event: InterruptableEvent, timeout: float
    ):
    ...
    def _initialize_command_socket(self, timeout=0.1):
        ...
        if self._command_socket is not None:
            raise InvalidStateError(
                "Cannot connect since existing command socket exists."
            )
        ...
        self._command_socket = self._connect_socket(
            self.logger, self._address, port, timeout
        )

        if self._command_socket is None:
            raise CommunicationError(
                "Command socket could not be created. Is the IP address correct?"
            )
    ...
    def _initialize_monitoring_socket(self, timeout):
        ...
        if self._monitor_socket is not None:
            raise InvalidStateError(
                "Cannot connect since existing monitor socket exists."
            )
        ...
        self._monitor_socket = self._connect_socket(
            self.logger, self._address, port, timeout
        )

        if self._monitor_socket is None:
            raise CommunicationError(
                "Monitor socket could not be created. Is the IP address correct?"
            )
    ...
    def _receive_welcome_message(
        self, message_queue: queue.Queue, from_command_port: bool
    ):
        ...
        while True:
            try:
                response: Message = message_queue.get(
            ...
            if (
                response.id == mx_st.MX_ST_CONNECTED
                or response.id == mx_st.MX_ST_GET_STATUS_ROBOT
            ):
                break
            ...
            if (time.monotonic() - start) > self.default_timeout:
                self.logger.error(
                    "No connect message received within timeout interval."
                )
                break
        ...
        if response.id == mx_st.MX_ST_CONNECTED:
            ...
            self._robot_info = RobotInfo(
                model="Meca500", revision=3, version="8.0.0.0-unknown-version"
            )
        else:
    ...
    def _initialize_command_connection(self):
        ...
        self._command_response_handler_thread = self._launch_thread(
            target=self._command_response_handler, args=()
        )
    ...
    def _initialize_monitoring_connection(self):
        ...
        self._monitor_handler_thread = self._launch_thread(
            target=self._monitor_handler, args=()
        )
    ...
    def _shut_down_queue_threads(self):
        ...
        if self._command_response_handler_thread is not None:
            try:
                ...
                self.logger.error(
                    "Error shutting down command response handler thread. " + str(e)
                )
        ...
        if self._monitor_handler_thread is not None:
            try:
                ...
                self.logger.error(
                    "Error shutting down monitor handler thread. " + str(e)
                )
    ...
    def _monitor_handler(self):
        ...
        while True:
            ...
            with self._main_lock:
                ...
                if (
                    response.id == mx_st.MX_ST_GET_POSE
                    and not self._robot_info.rt_message_capable
                ):
                    ...
                    if self._file_logger:
                        ...
                        self._file_logger.write_fields(
                            time.time_ns() / 1000, self._robot_rt_data
                        )
            ...
            with self._main_lock:
                ...
                if response.id == mx_st.MX_ST_NOT_ACTIVATED:
                    ...
                    self._robot_events.on_offline_program_started.abort(
                        f"Failed to start program: {response.data}"
                    )
                    ...
                    self._robot_events.on_offline_program_op_done.abort(
                        "Failed to list offline programs"
                    )
    ...
    def _handle_common_messages(self, response: Message):
        ...
        if response.id in robot_status_code_info:
            ...
            if code_info.is_error:
                self.logger.error(
                    f"Received robot error {code_info.code} ({code_info.name}): {response.data}"
                )
        else:
        ...
        if self._robot_info.rt_message_capable:
            ...
            if response.id == mx_st.MX_ST_GET_JOINTS:
                ...
                if self._tmp_rt_joint_pos:
                    self._robot_rt_data.rt_target_joint_pos.update_from_data(
                        timestamp, self._tmp_rt_joint_pos
                    )
                    self._tmp_rt_joint_pos = None
                if self._tmp_rt_cart_pos:
                    self._robot_rt_data.rt_target_cart_pos.update_from_data(
                        timestamp, self._tmp_rt_cart_pos
                    )
                    self._tmp_rt_cart_pos = None
            ...
            if response.id == mx_st.MX_ST_GET_JOINTS:
                self._robot_rt_data.rt_target_joint_pos.data = string_to_numbers(
                    response.data
                )
                ...
                self._robot_rt_data.rt_target_cart_pos.data = string_to_numbers(
                    response.data
                )
                ...
                self._robot_rt_data.rt_target_conf.data = string_to_numbers(
                    response.data
                )
                self._robot_rt_data.rt_target_conf.enabled = True

            elif response.id == mx_st.MX_ST_GET_CONF_TURN:
                self._robot_rt_data.rt_target_conf_turn.data = string_to_numbers(
                    response.data
                )
        ...
        if response.id == mx_st.MX_ST_GET_MOTION_STATUS:
            ...
            self._handle_ext_tool_sim_status(
                MxExtToolType.MX_EXT_TOOL_NONE
            )  # Legacy response (used by 8.4.4 and older)
    ...
    def _set_activated(self, activated: bool):
        ...
        """
        if (
            not self._first_robot_status_received
            or self._robot_status.activation_state != activated
        ):
            if activated:
    ...
    def _set_homed(self, homed: bool):
        ...
        if (
            not self._first_robot_status_received
            or self._robot_status.homing_state != homed
        ):
            if homed:
    ...
    def _set_sim_mode(self, sim_mode: bool):
        ...
        if (
            not self._first_robot_status_received
            or self._robot_status.simulation_mode != sim_mode
        ):
            if sim_mode:
            ...
            if (
                self._robot_events.on_activate_ext_tool_sim.is_set()
                != self._robot_status.simulation_mode
            ):
                # Sim mode was just disabled -> Also means external tool sim has been disabled
                self._handle_ext_tool_sim_status(
                    self._external_tool_status.sim_tool_type
                )
    ...
    def _set_recovery_mode(self, recovery_mode: bool):
        ...
        if (
            not self._first_robot_status_received
            or self._robot_status.recovery_mode != recovery_mode
        ):
            if recovery_mode:
    ...
    def _set_error_status(self, error_status: bool):
        ...
        if (
            not self._first_robot_status_received
            or self._robot_status.error_status != error_status
        ):
            if error_status:
    ...
    def _set_paused(self, paused: bool):
        ...
        if (
            not self._first_robot_status_received
            or self._robot_status.pause_motion_status != paused
        ):
            if paused:
    ...
    def _set_eob(self, eob: bool):
        ...
        if (
            not self._first_robot_status_received
            or self._robot_status.end_of_block_status != eob
        ):
            if eob:
    ...
    def _handle_robot_status_response(self, response: Message):
        ...
        if response.jsonData:
            ...
            jsonData = response.jsonData[MX_JSON_KEY_DATA]
            self._set_activated(
                jsonData[MX_JSON_KEY_STATUS_ROBOT_STATE]
                >= MxRobotState.MX_ROBOT_STATE_ACTIVATED
            )
            self._set_homed(
                jsonData[MX_JSON_KEY_STATUS_ROBOT_STATE]
                == MxRobotState.MX_ROBOT_STATE_RUN
            )
    ...
    def _handle_checkpoint_response(self, response: Message):
        ...
        if (
            checkpoint_id in self._user_checkpoints
            and self._user_checkpoints[checkpoint_id]
        ):
        ...
        # Check internal checkpoints.
            ...
            checkpoint_id in self._internal_checkpoints
            and self._internal_checkpoints[checkpoint_id]
        ):
