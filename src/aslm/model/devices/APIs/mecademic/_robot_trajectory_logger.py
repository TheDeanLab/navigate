...
class _RobotTrajectoryLogger:
    ...
    def __init__(
        ...
        serial_number_or_blank = (
            ("_serial_" + robot_info.serial) if robot_info.serial else ""
        )
        ...
        if fields is None:
            ...
            if robot_info.rt_message_capable:
                ...
                self.fields = {
                    "rt_target_joint_pos": robot_rt_data_to_real_time_monit[
                        "rt_target_joint_pos"
                    ][1],
                    "rt_target_cart_pos": robot_rt_data_to_real_time_monit[
                        "rt_target_cart_pos"
                    ][1],
                }
        else:
            for field in fields:
                for key, val in robot_rt_data_to_real_time_monit.items():
                    if (
                        isinstance(field, str) and field.lower() == val[1].lower()
                    ) or field == val[0]:
        ...
        self.data_dict: dict = (
            dict()
        )  # Key: timestamp, Value: List of all corresponding robot_rt_data values
        ...
        for attr in ["model", "revision", "version"]:
            self.robot_trajectories.robot_context.robot_information[0][
                attr
            ] = f"{getattr(robot_info, attr)}"
        ...
        for key, value in self.fields.items():
            if (
                ...
                self.expanded_fields.extend(
                    assemble_with_prefix(
                        value,
                        ["sim_model", "physical_model", "present", "homed", "error"],
                    )
                )
            elif key.endswith("rt_valve_state"):
                self.expanded_fields.extend(
                    assemble_with_prefix(value, ["valve1", "valve2"])
                )
            elif key.endswith("rt_gripper_state"):
                self.expanded_fields.extend(
                    assemble_with_prefix(
                        value, ["holding", "atpos", "closed", "opened"]
                    )
                )
    ...
    def write_fields(self, timestamp, robot_rt_data):
        ...
        for field in self.fields:
            ...
            if ts_data is None:
                continue
            self.data_dict[formatted_tim].extend(
                [f"{x:{self.element_width}}" for x in ts_data.data]
            )
    ...
    def end_log(self, ignore_checkpoints=True):
        ...
        self.robot_trajectories.robot_df_hist.output_dfs.append(
            pd.DataFrame.from_dict(
                self.data_dict, orient="index", columns=self.expanded_fields
            )
        )
