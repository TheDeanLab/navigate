...
class Robot(_Robot):
    ...
    def RunCallbacks(self):
        ...
        self._handle_callbacks(
            self.logger, self._callback_queue, self._robot_callbacks, timeout=0
        )
    ...
    def IsAllowedToMove(self) -> bool:
        ...
        with self._main_lock:
            can_move = self._robot_status.homing_state or (
                self._robot_status.activation_state
    ...
    def GripperOpen(self):
        ...
        if (
            ...
            and self._robot_info.gripper_pos_ctrl_capable
            and self.GetRtExtToolStatus().is_gripper()
        ):
    ...
    def SendCustomCommand(
        self, command: str, expected_responses: list[int] = None, timeout: float = None
    ) -> InterruptableEvent:
        ...
        return self._send_custom_command(
            command, expected_responses, timeout, skip_internal_check=True
        )
    ...
    def ListPrograms(self, timeout: float = None) -> dict:
        ...
        try:
            response = self._robot_events.on_offline_program_op_done.wait(
                timeout=timeout
            )
        except InterruptException as e:
            raise InvalidStateError(str(e))
    ...
    def GetRtExtToolStatus(
        ...
        if synchronous_update:
            self._send_sync_command(
                "GetRtExtToolStatus",
                self._robot_events.on_external_tool_status_updated,
                timeout,
            )
    ...
    def GetRtGripperState(
        ...
        if synchronous_update:
            self._send_sync_command(
                "GetRtGripperState",
                self._robot_events.on_gripper_state_updated,
                timeout,
            )
    ...
    def GetRtValveState(
        ...
        if synchronous_update:
            self._send_sync_command(
                "GetRtValveState", self._robot_events.on_valve_state_updated, timeout
            )
    ...
    def GetRtTargetJointPos(
        ...
        if synchronous_update:
            if self._robot_info.rt_message_capable:
                self._send_sync_command(
                    "GetRtTargetJointPos", self._robot_events.on_joints_updated, timeout
                )
            else:
                # This robot does not have GetRtTargetJointPos, use legacy GetJoints (but won't get timestamp)
                self._send_sync_command(
                    "GetJoints", self._robot_events.on_joints_updated, timeout
                )
        ...
        with self._main_lock:
            if include_timestamp:
                if not self._robot_info.rt_message_capable:
                    raise InvalidStateError(
                        "Cannot provide timestamp with current robot firmware or model."
                    )
                else:
    ...
    def GetJoints(self, synchronous_update: bool = False, timeout: float = None):
        ...
        if timeout is None:
            timeout = self.default_timeout
        return self.GetRtTargetJointPos(
    ...
    def GetRtTargetCartPos(
        ...
        if synchronous_update:
            if self._robot_info.rt_message_capable:
                self._send_sync_command(
                    "GetRtTargetCartPos", self._robot_events.on_pose_updated, timeout
                )
            else:
                # This robot does not have GetRtTargetCartPos, use legacy GetPose (but won't get timestamp)
                self._send_sync_command(
                    "GetPose", self._robot_events.on_pose_updated, timeout
                )
    ...
    def GetPose(
        ...
        if timeout is None:
            timeout = self.default_timeout
        return self.GetRtTargetCartPos(
            include_timestamp=False,
            synchronous_update=synchronous_update,
            timeout=timeout,
        )
    ...
    @disconnect_on_exception
    def SetExtToolSim(
        self, sim_ext_tool_type: int = MxExtToolType.MX_EXT_TOOL_MEGP25_SHORT
    ):
        ...
        if (
            sim_ext_tool_type != MxExtToolType.MX_EXT_TOOL_NONE
            and not self._robot_info.gripper_pos_ctrl_capable
        ):
            sim_ext_tool_type = 1
    ...
    def SetJointLimitsCfg(self, e: bool = True):
        ...
        if self._enable_synchronous_mode:
            response_event = self.SendCustomCommand(
                ...
                expected_responses=[
                    MxRobotStatusCode.MX_ST_SET_JOINT_LIMITS_CFG,
                    MxRobotStatusCode.MX_ST_CMD_FAILED,
                ],
            )
            if (
                response_event.wait(timeout=DEFAULT_WAIT_TIMEOUT).id
                == MxRobotStatusCode.MX_ST_CMD_FAILED
            ):
                raise MecademicException(
                    "Argument Error in Command : SetJointLimitsCfg"
                )
        else:
            with self._main_lock:
                self._check_internal_states()
    ...
    def SetJointLimits(self, n: int, lower_limit: float, upper_limit: float):
        ...
        if self._enable_synchronous_mode:
            response_event = self.SendCustomCommand(
                ...
                expected_responses=[
                    MxRobotStatusCode.MX_ST_SET_JOINT_LIMITS,
                    MxRobotStatusCode.MX_ST_CMD_FAILED,
                ],
            )
            if (
    ...
    def SetWorkspaceLimitsCfg(
        ...
        if self._enable_synchronous_mode:
            response_event = self.SendCustomCommand(
                f"SetWorkspaceLimitsCfg({severity},{mode})",
                expected_responses=[
            ...
            if (
                ...
                raise MecademicException(
                    "Argument Error in Command : SetWorkspaceLimitsCfg"
                )
        else:
            with self._main_lock:
                self._check_internal_states()
    ...
    def SetWorkspaceLimits(
        ...
        if self._enable_synchronous_mode:
            response_event = self.SendCustomCommand(
                ...
                expected_responses=[
                    MxRobotStatusCode.MX_ST_SET_WORKSPACE_LIMITS,
                    MxRobotStatusCode.MX_ST_CMD_FAILED,
                ],
            )
            if (
                ...
                raise MecademicException(
                    "Argument Error in Command : SetWorkspaceLimits"
                )
        else:
            with self._main_lock:
                self._check_internal_states()
                self._send_command(
                    "SetWorkspaceLimits", [x_min, y_min, z_min, x_max, y_max, z_max]
                )
    ...
    def SetToolSphere(self, x: float, y: float, z: float, r: float):
        ...
        if self._enable_synchronous_mode:
            response_event = self.SendCustomCommand(
                ...
                expected_responses=[
                    MxRobotStatusCode.MX_ST_SET_TOOL_SPHERE,
                    MxRobotStatusCode.MX_ST_CMD_FAILED,
                ],
            )
            if (
    ...
    def SetTorqueLimitsCfg(
        ...
        """
        severity_int = (
            TORQUE_LIMIT_SEVERITIES[severity] if type(severity) == str else severity
        )
        skip_acceleration_int = 1 if skip_acceleration else 0
        self._send_motion_command(
            "SetTorqueLimitsCfg", [severity_int, skip_acceleration_int]
        )
    ...
    def GetGripperRange(self, timeout: float = None) -> Tuple[float, float]:
        ...
        response = self._send_custom_command(
            "GetGripperRange",
    ...
    def StartLogging(
        ...
        self._file_logger = mx_traj._RobotTrajectoryLogger(
            self._robot_info,
            self._robot_rt_data,
            fields,
            file_name=file_name,
    ...
    def EndLogging(self, keep_captured_trajectory: bool = False) -> str:
        ...
        if self._robot_info.rt_message_capable:
            if self._monitoring_interval_to_restore != self._monitoring_interval:
                ...
                self._set_monitoring_interval_internal(
                    self._monitoring_interval_to_restore
                )
