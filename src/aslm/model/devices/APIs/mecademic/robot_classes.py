...
class InterruptableEvent:
    ...
    def __init__(
        self, id=None, data=None, abort_on_error=False, abort_on_clear_motion=False
    ):
        self._id = id
        self._data = data
        ...
        if self._interrupted:
            if self._interrupted_msg != "":
                ...
                raise InterruptException(
                    "Event interrupted, possibly because event will never be triggered."
                )
...
class RobotInfo:
    ...
    def from_command_response_string(cls, input_string: str):
        ...
        ROBOT_CONNECTION_STRING = (
            r"Connected to (?P<model>[\w|-]+) ?R?(?P<revision>\d)?"
        )
        ROBOT_CONNECTION_STRING += (
            r"(?P<virtual>-virtual)?( v|_)?(?P<version>\d+\.\d+\.\d+)"
        )
        ...
        try:
            ...
            raise ValueError(
                f'Could not parse robot info string "{input_string}", error: {exception}'
            )
    ...
    def get_serial_digit(self) -> int:
        ...
        if len(serial_split) != 2:
            raise ValueError(
                f'Invalid serial number string received: {self.serial}, expecting "M500-1234"'
            )
...
class RobotRtData:
    ...
    def __init__(self, num_joints: int):
        ...
        self.rt_external_tool_status = TimestampedData.zeros(
            5
        )  # microseconds timestamp, sim tool type, physical tool type, activated, homed, error
        ...
        self.rt_gripper_force = TimestampedData.zeros(
            1
        )  # microseconds timestamp, gripper force [%]
        self.rt_gripper_pos = TimestampedData.zeros(
            1
        )  # microseconds timestamp, gripper position [mm]
        ...
        self.rt_checkpoint = TimestampedData.zeros(
            1
        )  # microseconds timestamp, checkpointId
...
class ExtToolStatus:
    ...
    def __str__(self) -> str:
        return (
            f"Sim tool type: {self.sim_tool_type}, Physical tool type: {self.physical_tool_type}, "
    ...
    def current_tool_type(self) -> int:
        ...
        return (
            self.sim_tool_type
            if self.sim_tool_type != MxExtToolType.MX_EXT_TOOL_NONE
            else self.physical_tool_type
        )
    ...
    def is_gripper(self, physical: bool = False) -> bool:
        ...
        return tool_type in [
            MxExtToolType.MX_EXT_TOOL_MEGP25_SHORT,
            MxExtToolType.MX_EXT_TOOL_MEGP25_LONG,
        ]
...
class GripperState:
    ...
    def __str__(self):
        return (
            f"holding={self.holding_part} pos_reached={self.target_pos_reached} "
