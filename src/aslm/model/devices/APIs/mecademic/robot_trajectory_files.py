...
class RobotDfHist:
    ...
    def build_from_dict(self, df_dict):
        ...
        for key, df in df_dict.items():
            result, self.input_dfs = self.insert_in_list_from_dict(
                key, df, self.input_dfs, self.base_input_name
            )
            result, self.output_dfs = self.insert_in_list_from_dict(
                key, df, self.output_dfs, self.base_output_name
            )
            if not result:
                self.mid_dfs[key] = df
    ...
    def insert_in_list_from_dict(self, key, df, attr_list, list_prefix_func):
        ...
        return (
            result,
            attr_list,
        )  # It seems if we pass attribute as argument, even if attribute is passed by reference,
    ...
    def to_file(self, filename, file_path=None):
        ...
        with TemporaryDirectory(dir=current_dir) as root_dir:
            ...
            for key, df in self.robot_df_hist.make_dict().items():
                RobotDfHist.to_file(
                    df, PurePath.joinpath(PurePath(root_dir), "_".join([filename, key]))
                )
        ...
        with TemporaryDirectory(dir=current_dir) as base_dir:
            ...
            for file in files:
                if file.name.endswith(".csv"):
                    base_name_file = PurePath(
                        file.name
                    ).stem  # Keep file name without extension
                    base_name_file = base_name_file.removeprefix(base_name + "_")
                    df_dict[base_name_file] = RobotDfHist.from_file(
