...
def main():
    ...
    cam.prog_scan_mode = (
        1  # Scan mode options: {'Auto': 0, 'Line Delay': 1, 'Scan Width': 2}
    )
    cam.prog_scan_dir = (
        0  # Scan direction options: {'Down': 0, 'Up': 1, 'Down/Up Alternate': 2}
    )
    ...
    def calculate_ASLMparameters(desired_exposuretime):
        ...
        ASLM_lineExposure = int(
            np.ceil(desired_exposuretime / (1 + nbrows / ASLM_scanWidth))
        )
        ASLM_line_delay = (
            int(
                np.ceil(
                    (desired_exposuretime - ASLM_lineExposure) / (nbrows * linedelay)
                )
            )
            - 1
        )
    ...
    while True:
        ...
        frame["pixel_data"] = cv2.resize(
            frame["pixel_data"], dim, interpolation=cv2.INTER_AREA
        )
