...
def main():
    ...
    while cnt < NUM_FRAMES:
        ...
        for roi_index in range(num_rois):
            ...
            print(
                "ROI:{:2} ({:4}, {:4}) ({:4}, {:4})\tMin:{}\tMax:{}\tAverage:{:.0f}\tFrame Rate: {:.1f}\tFrame Count: {:.0f}".format(
                    roi_index, s1, p1, s2, p2, low, high, average, fps, frame_count
                )
            )
            ...
            img = cv2.convertScaleAbs(
                frame["pixel_data"][roi_index], alpha=alpha, beta=beta
            )
