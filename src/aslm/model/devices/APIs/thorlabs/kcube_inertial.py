...
class TLDLLError(Exception):
    r"""Exception for general Thorlabs DLL control errors."""


class TLMotorDLLError(Exception):
    r"""Exception for motor-specific Thorlabs DLL errors."""


class FT_Status(IntEnum):
    r"""The following errors are generated from the FTDI communications module, supporting code or device libraries."""
    FT_OK = 0  # No error
...
class TL_DLL_Error(IntEnum):
    r"""The following errors are general errors generated by all DLLs. """
...
TL_DLL_Error_Description = {
    ...
    TL_DLL_Error.TL_DISCONNECTING: "The function could not be completed because the device is disconnected.",
...
class Motor_DLL_Error(IntEnum):
    r"""The following errors are motor specific errors generated by the Motor DLLs."""
...
def TLI_BuildDeviceList():
    r"""This function builds an internal collection of all devices found on the USB that are not currently open.
...
def TLI_GetDeviceListExt():
    r"""Get the entire contents of the device list.
...
def KIM_Open(serial_no):
    r"""Open the device for communications.
    ...
    return __dll.KIM_Open(serial_no.encode('ascii'))
...
def KIM_Close(serial_no):
    r"""Disconnect and close the device.
    ...
    __dll.KIM_Close(serial_no.encode('ascii'))
...
def KIM_StartPolling(serial_no, milliseconds):
    r"""Starts the internal polling loop which continuously requests position and status.
    ...
    """
    return bool(__dll.KIM_StartPolling(serial_no.encode('ascii'), milliseconds))
...
def KIM_StopPolling(serial_no):
    r"""Stops the internal polling loop.
    ...
    None
    """
    __dll.KIM_StopPolling(serial_no.encode('ascii'))
...
def KIM_GetCurrentPosition(serial_no, channel):
    ...
    return __dll.KIM_GetCurrentPosition(serial_no.encode('ascii'), channel)
...
def KIM_MoveAbsolute(serial_no, channel, position):
    ...
    return __dll.KIM_MoveAbsolute(serial_no.encode('ascii'), channel, position)
...
def KIM_MoveStop(serial_no, channel):
    ...
    return __dll.KIM_MoveStop(serial_no.encode('ascii'), channel)
