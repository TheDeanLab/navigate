...
class CameraMapSettingPopup(PopUp):
    ...
    def __init__(self, root, name="Camera Map Settings", size='+320+180', top=True, transient=True, *args, **kwargs):
        ...
        self.inputs['file_name'] = tk.Entry(self.content_frame, textvariable=self.file_name)
        self.inputs['file_name'].grid(row=0, column=1, sticky=tk.NSEW, padx=(0, 5), pady=(15, 0))
        ...
        self.inputs['camera'].grid(row=0, column=4, sticky=tk.NSEW, padx=(0, 5), pady=(15, 0))
        ...
        canvas.draw()
        canvas.get_tk_widget().grid(row=1, column=0, columnspan=6, sticky=tk.NSEW, padx=(5, 5), pady=(5, 5))
