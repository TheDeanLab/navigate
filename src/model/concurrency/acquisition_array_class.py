...
class acquisition_arrays:
    ...
    def get_highresASLM_preview_array(self):
        ...
        basic_unit = np.zeros(
            (totallength, NI_board_parameters.ao_nchannels), np.dtype(
                np.float64))

        basic_unit[self.model.ao.s2p(
            0.001):totallength, self.model.current_laser] = 4
        basic_unit[self.model.ao.s2p(0.001): self.model.ao.s2p(
            0.002), NI_board_parameters.highres_camera] = 4.  # high-res camera
        ...
        goingdownpoints = self.model.ao.s2p(
            0.001 + self.model.ASLM_acquisition_time / 1000 + 0.02) - goinguppoints
    ...
    def get_lowRes_StackAq_array(self, current_laserline):
        ...
        readout_time = (nb_rows + 1) * \
            Camera_parameters.lowres_line_digitization_time

        # prepare voltage array
        # calculate minimal unit duration and set up array
        minimal_trigger_timeinterval = self.model.exposure_time_LR / 1000 + \
            readout_time / 1000 + self.model.delay_cameratrigger + 0.001
        basic_unit = np.zeros(
        ...
        basic_unit[self.model.ao.s2p(self.model.delay_cameratrigger):self.model.ao.s2p(
            self.model.delay_cameratrigger + 0.001), NI_board_parameters.lowres_camera] = 4.  # low res camera
        basic_unit[0:self.model.ao.s2p(0.002),
                   NI_board_parameters.stage] = 4.  # stage
        basic_unit[self.model.ao.s2p(self.model.delay_cameratrigger):self.model.ao.s2p(
            self.model.exposure_time_LR / 1000), current_laserline] = 4.  # laser
        # remote mirror
        basic_unit[:, NI_board_parameters.voicecoil] = self.model.ASLM_staticLowResVolt
    ...
    def get_highResSPIM_StackAq_array(self, current_laserline):
        ...
        # prepare voltage array
        # calculate minimal unit duration and set up array
        minimal_trigger_timeinterval = self.model.exposure_time_HR / 1000 + \
            readout_time / 1000 + self.model.delay_cameratrigger + 0.001
        basic_unit = np.zeros(
            (self.model.ao.s2p(minimal_trigger_timeinterval),
             NI_board_parameters.ao_nchannels),
        ...
        # set voltages in array - camera, stage, remote mirror, laser
        basic_unit[self.model.ao.s2p(self.model.delay_cameratrigger):self.model.ao.s2p(
            self.model.delay_cameratrigger + 0.001), NI_board_parameters.highres_camera] = 4.  # highrescamera - ao0
        basic_unit[0:self.model.ao.s2p(0.002),
                   NI_board_parameters.stage] = 4.  # stage
        # remote mirror
        basic_unit[:, NI_board_parameters.voicecoil] = self.model.ASLM_staticHighResVolt
        basic_unit[self.model.ao.s2p(self.model.delay_cameratrigger):self.model.ao.s2p(
            self.model.exposure_time_HR / 1000), current_laserline] = 4.  # laser
    ...
    def get_highResASLM_StackAq_array(self, current_laserline):
        ...
        readout_time = (nb_rows + 1) * Camera_parameters.highres_line_digitization_time * \
            self.model.ASLM_line_delay  # +1 for the reset time at the first row before the start
        ...
        basic_unit[0:self.model.ao.s2p(
            0.002), NI_board_parameters.stage] = 4.  # stage - move to position
        basic_unit[self.model.ao.s2p(self.model.delay_cameratrigger):self.model.ao.s2p(
            self.model.delay_cameratrigger + self.model.ASLM_acquisition_time / 1000), current_laserline] = 4.  # laser
        ...
        goinguppoints = self.model.ao.s2p(
            ...
            0.001 +
            self.model.ASLM_delaybeforevoltagereturn +
            self.model.ASLM_acquisition_time /
            1000)
        goingdownpoints = self.model.ao.s2p(
            minimal_trigger_timeinterval) - goinguppoints
        sawtooth_array[0:goinguppoints] = np.linspace(
            self.model.ASLM_from_Volt, self.model.ASLM_to_Volt, goinguppoints)
        sawtooth_array[goinguppoints:] = np.linspace(
            self.model.ASLM_to_Volt, self.model.ASLM_from_Volt, goingdownpoints)
