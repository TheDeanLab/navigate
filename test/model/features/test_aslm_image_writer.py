...
def test_zarr_byslice():
    ...
    f_shape = (
        (X < 0.2) | ((Y > 0.4) & (Y < 0.6) & (X < 0.6)) | ((Y < 0.2) & (X < 0.8))
    )  # 3D numpy array
    ...
    dummy_model.experiment.MicroscopeState["channels"] = {
        "channel_2": {"is_selected": True},
        "channel_3": {"is_selected": True},
        "channel_4": {"is_selected": True},
    }

    num_of_chans = len(
        dummy_model.experiment.MicroscopeState["channels"].keys()
    )  # n chans
